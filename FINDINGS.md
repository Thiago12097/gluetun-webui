...
|---|---|---|---|
| S-03 | 🔵 Low | `src/server.js` | **Instance URLs are not validated at startup.** Values from `GLUETUN_{N}_URL` and legacy `GLUETUN_CONTROL_URL` are used verbatim as fetch targets. A malformed or attacker-controlled value could target arbitrary internal addresses. Validate each URL with `new URL()` in `parseInstances()` and exit on failure. |
...
<details>
<summary>Click to expand — 37 issues resolved</summary>
...
1. **S-03** — Validate all instance URLs at startup with `new URL()` in `parseInstances()`
3. **S-08** — Store `app.listen()` as `const server`, then add graceful shutdown handler
4. **C-08** — Remove dead CSS rules (`#banner-title`, `#banner-sub`, `.card-header h2`, `.grid`)
5. **C-07** — Remove unused `instance-tabs` nav or implement tab switching
6. **C-10** — Consolidate `staticLimiter` and `uiLimiter` into a single static content limiter
7. **C-09** — Consolidate `$`/`setText`/`setEl` into one utility function
8. **C-06** — Scope `express.json()` to PUT routes only
9. **N-03** — Add `type="button"` to all `<button>` elements
10. **C-12** — Add `Array.isArray()` guard on `/api/instances` response
11. **C-04** — Add tests for `gluetunFetch`, `updatePanel`, and `updatePanelError`
12. **C-05** — Replace `innerHTML` spinner with `createElement`
13. **D-01** — Add container resource limits to `docker-compose.yml`
14. **C-03** — Plan Express 5 migration (review changelog for breaking changes first)
...
- **S-03, S-05, S-06, S-08** — Security findings unchanged.
